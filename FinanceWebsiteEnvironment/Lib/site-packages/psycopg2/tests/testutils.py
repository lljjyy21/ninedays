...
class ConnectingTestCase(unittest.TestCase):
    """A tests case providing connections for tests.

    A connection for the tests is always available as `self.conn`. Others can be
    ...
    def tearDown(self):
        # close the connections used in the tests
    ...
    def repl_connect(self, **kwargs):
        ...
        Should raise a skip tests if not available, but guard for None on
        old Python versions.
        """
        if repl_dsn is None:
...
def skip_if_no_uuid(f):
    """Decorator to skip a tests if uuid is not supported by Py/PG."""
    @wraps(f)
...
def skip_if_tpc_disabled(f):
    """Skip a tests if the server has tpc support disabled."""
    @wraps(f)
    def skip_if_tpc_disabled_(self):
        ...
        if not mtp:
            return self.skipTest(
                ...
                "set max_prepared_transactions to > 0 to run the tests")
        return f(self)
...
def skip_if_no_iobase(f):
    """Skip a tests if io.TextIOBase is not available."""
    @wraps(f)
...
def skip_before_postgres(*ver):
    """Skip a tests on PostgreSQL before a certain version."""
    ver = ver + (0,) * (3 - len(ver))
...
def skip_after_postgres(*ver):
    """Skip a tests on PostgreSQL after (including) a certain version."""
    ver = ver + (0,) * (3 - len(ver))
...
def skip_before_libpq(*ver):
    """Skip a tests if libpq we're linked to is older than a certain version."""
    ver = ver + (0,) * (3 - len(ver))
...
def skip_after_libpq(*ver):
    """Skip a tests if libpq we're linked to is newer than a certain version."""
    ver = ver + (0,) * (3 - len(ver))
...
def skip_before_python(*ver):
    """Skip a tests on Python before a certain version."""
...
def skip_from_python(*ver):
    """Skip a tests on Python after (including) a certain version."""
...
def skip_if_no_superuser(f):
    """Skip a tests if the database user running the tests is not a superuser"""
    @wraps(f)
...
def skip_if_windows(f):
    """Skip a tests if run on windows"""
    @wraps(f)
...
def slow(f):
    ...
    def slow_(self):
        if os.environ.get('PSYCOPG2_TEST_FAST'):
            return self.skipTest("slow tests")
        return f(self)
    return slow_
