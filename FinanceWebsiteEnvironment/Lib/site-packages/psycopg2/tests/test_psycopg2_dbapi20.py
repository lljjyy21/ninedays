...
# test_psycopg2_dbapi20.py - DB API conformance tests for psycopg2
#
...
class Psycopg2Tests(dbapi20.DatabaseAPI20Test):
    ...
    lower_func = 'lower' # For stored procedure tests

    def test_callproc(self):
        # Until DBAPI 2.0 compliance, callproc should return None or it's just
        # misleading. Therefore, we will skip the return value tests for
        # callproc and only perform the fetch tests.
        #
        # For what it's worth, the DBAPI2.0 test_callproc doesn't actually
        # tests for DBAPI2.0 compliance! It doesn't check for modified OUT and
